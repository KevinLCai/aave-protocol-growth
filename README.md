...
- **Runtime.** Notebooks 00, 01, and 04 run in well under a minute. Notebook 02 is the slow one — two of its cells page through full `Transfer` event history in ≤10,000-block windows (the public endpoint's enforced range cap, error `-32602`): the first cell scans both the sUSDe aToken and USDe debt token back to block 0 (~5,100 windows), and Section 2.4 separately scans USDe's full mint history once, reused across all 10 addresses it checks (~2,569 windows more). Both use a thread pool (`fetch_logs_parallel`, 10 workers) and print progress every 200 windows with a running ETA, so you're not staring at a silent cell — expect roughly **10–20 minutes** on the public endpoint (not rate limiting, just latency × request count — a VPN won't help). The rest of the notebook (batching ~18,000 balance/account-data calls through Multicall3) adds only another minute or two.
- **No API keys required.** All notebooks, including 02's log fetching, run against the public `https://eth.drpc.org` endpoint set at the top of each setup cell — no `.env` or private RPC key needed. Etherscan lookups referenced in the markdown (e.g. wallet tx history, name tags) were done manually in a browser and aren't scripted.
